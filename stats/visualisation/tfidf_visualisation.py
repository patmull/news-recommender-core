...
class TfIdfVisualizer:
    ...
    def prepare_for_heatmap(self, tfidf_vectors, text_titles, tfidf_vectorizer):
        ...
        tfidf_df = tfidf_df.rename(columns={0: 'terms_frequencies', 'level_0': 'document', 'level_1': 'term', 'level_2': 'term'})
        print("tfidf_df.columns:")
        print(tfidf_df.columns)
        tfidf_df.sort_values(by=['document', 'terms_frequencies'], ascending=[True, False]).groupby(['document']).head(10)
        self.top_tfidf = tfidf_df.sort_values(by=['document', 'terms_frequencies'], ascending=[True, False]).groupby(['document']).head(10)
        test_word_1 = "žena"
    ...
    def plot_tfidf_heatmap(self):
        ...
        top_tfidf_plus_rand['terms_frequencies'] = top_tfidf_plus_rand['terms_frequencies'] + np.random.rand(self.top_tfidf.shape[0]) * 0.0001
