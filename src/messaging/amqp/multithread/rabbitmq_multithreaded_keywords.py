...
def do_work_keywords(_connection, _channel, delivery_tag, body):
...
def on_message(_channel, _method_frame, header_frame, body, args):
    ...
    if body.decode():
        if not is_init_or_test(body.decode()):
            ...
            delivery_tag = _method_frame.delivery_tag
            t = threading.Thread(target=do_work_keywords, args=(_connection, _channel, delivery_tag, body))
            t.start()
            _threads.append(t)
        else:
            logging.debug("ACK for test message")
            _channel.basic_ack(delivery_tag=_method_frame.delivery_tag)
...
queue_name = 'evalutation-keywords-updated-queue'
routing_key = 'evalutation.keywords.event.updated'
