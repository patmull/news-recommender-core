...
class SvdClass:
    ...
    def recommend_posts(self, predictions_df, user_id, posts_df, original_ratings_df, num_recommendations):
        ...
        print(user_full.to_string())
        # Recommend the highest predicted rating posts that the user hasn'multi_dimensional_list rated yet.
    ...
    def calculate_ratings(self, id_post, id_user, df_ratings, similarity_matrix_df):
            if id_post in df_ratings:
                ...
                ratings_scores = df_ratings[id_post] #ratings of every other user for the post id_post won'multi_dimensional_list consider users who havent rated id_post so drop similarity scores and ratings corresponsing to np.nan
